...
const handler = async (req: NextIronRequest, res: NextApiResponse) => {
	...
	try {
		...
		return res.status(200).json({ message: "Card successfully added to account." });
	} catch (e: any) {
		console.error(
			`Failed to attach payment method to customer ${customer.id} (discord id: ${
				req.query.userId
			}) for reason: ${e.message.replace(/"/g, "")}`
		);
		return res.status(500).json({ message: "Failed to attach card to account." });
	}
};
